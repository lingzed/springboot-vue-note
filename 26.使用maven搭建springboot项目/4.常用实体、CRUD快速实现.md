...
在插入时，可以使用【parameterType】属性来指定实体类型，一般在新增方法中需要传入一个实体：

![image-20241107191916942](assets/image-20241107191916942.png)

而我们插入用到的字段是实体中的属性，如果加上【parameterType】，那么在`<insert>`中可以直接使用`#{属性}`的形式，而不需`#{对象.属性}`。如：

![image-20241107192117064](assets/image-20241107192117064.png)

在使用`#{属性}`的前提下，【parameterType】是否省略：

- 当方法形参只有一个时，可以省略，mybatis会自动推测实体类型。
- 当方法形参有多个时，不能省略。
- 当方法形参是集合时，不能省略，因为实体一般包含在集合中(泛型)。
- 当方法形参有多个时，不能省略。
